...
func updateDNS(c *config.DNS, generalIPv6 bool) {
	...
	r := dns.NewResolver(dns.Config{
		...
		OptimisticCacheTTL:       c.OptimisticCacheTTL,
		OptimisticCacheAnswerTTL: c.OptimisticCacheAnswerTTL,
		Prefetch:                 c.Prefetch,
		PrefetchScanInterval:     c.PrefetchScanInterval,
		PrefetchThreshold:        c.PrefetchThreshold,
		PrefetchMinRefreshes:     c.PrefetchMinRefreshes,
		MinTTL:                   c.MinTTL,
		MaxTTL:                   c.MaxTTL,
		PersistCache:             true,
	})
